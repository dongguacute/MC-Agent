...
public class Agent {
    ...
    public static void handleAgentPrompt(ServerPlayer player, String prompt) {
        if (prompt == null || prompt.trim().isEmpty()) {
            player.sendSystemMessage(AgentMessaging.i18n("command.modid.agent.chat.prompt.empty", "请输入提示词，例如：/agent ask 帮我总结今天任务").withStyle(ChatFormatting.YELLOW));
            return;
        }

        AgentConfig currentConfig = AgentState.config;
        if (currentConfig.apiKey().isBlank()) {
            player.sendSystemMessage(AgentMessaging.i18n("command.modid.agent.chat.api_key.missing", "未配置 API Key，请编辑 %s 中的 api_key", AgentConstants.CONFIG_FILE_NAME).withStyle(ChatFormatting.RED));
            return;
        }
        ...
        if (parts.size() == 1) {
            if (MinecraftWiki.looksLikeWikiKnowledgeQuestion(safePrompt)) {
                ...
                player.sendSystemMessage(AgentMessaging.i18n("command.modid.agent.chat.requesting", "[%s] 思考中...", AgentConstants.AGENT_DISPLAY_NAME).withStyle(ChatFormatting.DARK_GRAY));
            }
        } else {
            player.sendSystemMessage(
                    AgentMessaging.i18n(
        ...
        for (int i = 0; i < parts.size(); i++) {
            ...
            AgentPromptHandlers.enqueuePlayerAiTask(player, () -> {
                ...
                if (MinecraftWiki.looksLikeWikiKnowledgeQuestion(partPrompt)) {
                    ...
                    AgentMessaging.sendToMainThread(
                            player,
                            AgentMessaging.i18n("command.modid.agent.chat.requesting", "[%s] 思考中...", AgentConstants.AGENT_DISPLAY_NAME).withStyle(ChatFormatting.DARK_GRAY)
                    );
                }
    ...
    public static void handleWikiSearch(ServerPlayer player, String query) {
        if (query == null || query.trim().isEmpty()) {
            player.sendSystemMessage(AgentMessaging.i18n("command.modid.agent.search.prompt.empty", "请输入搜索关键词，例如：/agent search 末影龙").withStyle(ChatFormatting.YELLOW));
            return;
        }
        AgentConfig currentConfig = AgentState.config;
        if (currentConfig.apiKey().isBlank()) {
        ...
        AgentPromptHandlers.enqueuePlayerAiTask(player, () -> {
            ...
            if (excerpt == null || excerpt.isBlank()) {
                AgentMessaging.sendToMainThread(player, AgentMessaging.i18n("command.modid.agent.wiki.empty", "[%s] 未在 Wiki 找到可用摘要，请换关键词重试。", AgentConstants.AGENT_DISPLAY_NAME).withStyle(ChatFormatting.RED));
                return;
            }
            AgentMessaging.sendToMainThread(
                    player,
